...
class Character(metaclass=ABCMeta):
    ...
    # 추상 메서드
    # 파생 클래스는 반드시 attack() 과 get_damage() 메서드를 오버라이딩해야 한다.
    @abstractmethod
...
# 불과 얼음 몬스터는 모두 Character 클래스에서 추가된 attack_kind 멤버를 가지고 있으며
...
class Monster(Character):
    ...
    # 플레이어가 불 공격을 할때
    # 공격받은 객체가 얼음 몬스터라면 체력이 깍이고
    # 공격받은 객체가 불 몬스터라면 오히려 체력이 늘어난다

    def get_damage(self, power, attack_kind):
        '''
        몬스터는 자신과 타입이 같은 공격을 당하면 오히려 체력이 늘어난다
        조심해서 공격하세요
        :param power:
        :param attack_kind:
        :return:
...
if __name__ == '__main__':
    ...
    for monster in monsters:
        ...
        # 아이스 몬스터가 공격할 떄는 공격력이 절반 (10)만 깍이고
        # 파이어 몬스터가 공격할 떄는 공격력이 그대로 (20) 깍인다
